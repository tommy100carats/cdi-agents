...
- Périmètre (rules/criteres.md § 1) : CDI, géographie, familles, exclusions fermes. Lance `python3 -m pipeline.cli exclusions <fichier>` sur le même fichier texte : exclue = true → Écartée, Raison = le motif renvoyé (phrase citée : diplôme d'ingénieur ou d'informatique exigé, code au coeur du poste, R16). Les autres exclusions (outbound à froid, profil Data / ML / Software, LangChain, AWS) = Écartée avec la raison exacte citée de l'annonce.
