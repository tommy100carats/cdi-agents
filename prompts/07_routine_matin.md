...
ÉTAPE 3, VÉRIFICATION PAR CODE. Écris les liens dans liens.json et lance `python3 -m pipeline.cli links liens.json`. active = true → retenue, Vérifié actif le = aujourd'hui (update_properties) ; false → Statut « En veille », Notes en fin « ⚠️ lien mort le JJ/MM (Routine_Matin), preuve : … », Écrit par = Routine_Matin ; null → non retenue pour l'email (tu peux chercher un lien ATS et le revérifier ; sinon écarte et signale). Garde les 5 premières offres qui passent ; moins de 5 : envoie moins et dis-le. Jamais une offre non confirmée dans l'email.
...
ÉTAPE 5, JOURNAL. Lues = candidates ; Écrites = fiches passées en veille ; Écartées = liens morts + non vérifiables ; Contrôles : « liens vérifiés N (actifs X, morts Y, non vérifiables Z) ».

Ce que tu ne fais jamais : envoyer une offre non vérifiée par le script, créer ou supprimer une fiche, écrire à un tiers, plus d'un email.
